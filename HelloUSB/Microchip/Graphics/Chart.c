			...
			if (GetState(pCh, CH_DRAW_DATA)) {
				...
				if (GetState(pCh, CH_BAR)) {
					...
					// get the starting x position where redraw will take place
	   	    		h = xStart-GetTextWidth(tempXchar, pCh->hdr.pGolScheme->pFont)-
					...
					if (GetState(pCh, CH_3D_ENABLE)) {
						...
						if (GetState(pCh, CH_BAR_HOR) == CH_BAR_HOR) {
							...
							//if (((GetState(pCh, CH_LEGEND)) && (ChGetShowSeriesCount(pCh) != 1)) || (GetState(pCh, CH_VALUE))) {
					...
					Bar(pCh->hdr.left+GOL_EMBOSS_SIZE, 
						...
						ctr+z+(GetTextWidth(tempXchar, pCh->hdr.pGolScheme->pFont)*8), 
		...
		case BAR_DATA_DRAW_VALUE:
			...
			if (GetState(pCh, CH_3D_ENABLE)) {
				if (GetState(pCh, CH_BAR_HOR) == CH_BAR_HOR) { 
					...
					MoveTo(	x+barDepth+((temp-GetTextWidth(&tempStr[STR_CHAR_CNT-j], pCh->hdr.pGolScheme->pFont))>>1)+(temp*varCtr), 
							(yStart-1)-dTemp-GetTextHeight(pCh->hdr.pGolScheme->pFont)); 
				}
			} else {
				if (GetState(pCh, CH_BAR_HOR) == CH_BAR_HOR) { 
					MoveTo(	xStart+dTemp+3,
							y-(temp*varCtr)-((temp+GetTextHeight(pCh->hdr.pGolScheme->pFont))>>1)); 
				} else {
					MoveTo(	x+((temp-GetTextWidth(&tempStr[STR_CHAR_CNT-j], pCh->hdr.pGolScheme->pFont))>>1)+(temp*varCtr), 
							(yStart-1)-dTemp-(GetTextHeight(pCh->hdr.pGolScheme->pFont))); 
				}
			}
...
chrt_pie_prep:
        case PIE_PREP:
			...
			*/
			...
			if (GetState(pCh, CH_PERCENT)) {
				// 7 is derived from "X:100%," - five possible characters 
				y = (7*GetTextWidth((XCHAR*)tempXchar, pCh->hdr.pGolScheme->pFont));
			} else
				y = 0;
