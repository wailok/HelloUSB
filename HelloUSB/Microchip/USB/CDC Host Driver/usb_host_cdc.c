...
#define _USBHostCDC_SetNextState()              { deviceInfo[i].state = (deviceInfo[i].state & STATE_MASK) + NEXT_STATE; }
#define _USBHostCDC_SetNextSubState()           { deviceInfo[i].state += NEXT_SUBSTATE; }
...
//                                                        usbDeviceInterfaceTable.EventHandler( deviceInfo[i].deviceAddress, EVENT_CDC_TRANSFER, NULL, 0 );     \
//                                                    }
#endif
...
USB_CDC_DEVICE_INFO           deviceInfo[USB_MAX_CDC_DEVICES] __attribute__ ((aligned)); // This structure holds all the information of device collected during enumeration
...
BYTE    USBHostCDCDeviceStatus( BYTE deviceAddress )
{
    ...
    for (i=0; (i<USB_MAX_CDC_DEVICES) && (deviceInfo[i].deviceAddress != deviceAddress); i++);
            ...
            case STATE_INITIALIZE_DEVICE:
                switch (deviceInfo[i].state & SUBSTATE_MASK)
                {
                    ...
                    case SUBSTATE_DEVICE_ENUMERATED:
                        USBHostCDCInitAddress( deviceInfo[i].deviceAddress,0 ); // Initialize device address for application use 
                   ...
                   case SUBSTATE_GET_LINE_CODING:
                       ...
                       if(!USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                            USB_CDC_GET_LINE_CODING, 0 , deviceInfo[i].commInterface.interfaceNum , USB_CDC_LINE_CODING_LENGTH, (BYTE*)&CDC_DEV_LINE_CODING_Buffer,
                        ...
                        if (USBHostTransferIsComplete( deviceInfo[i].deviceAddress, 0, &errorCode, &byteCount ))
                        {
                            ...
                            else
                            {
                                ...
                                if(0 == memcmp(&CDC_DEV_LINE_CODING_Buffer, PTR_HOST_LINE_CODING_BUFFER, USB_CDC_LINE_CODING_LENGTH))
                                {
                                    ...
                                    deviceInfo[i].state = STATE_INITIALIZE_DEVICE | SUBSTATE_SET_CONTROL_LINE_STATE;
                                }
                                else
                                {
                                    // if data not as expected goto Set Line coding
                                    deviceInfo[i].state = STATE_INITIALIZE_DEVICE | SUBSTATE_SET_LINE_CODING;
                                }
                            }
                        }
                        break;
                   ...
                   case SUBSTATE_SET_LINE_CODING:
                       ...
                       if(!USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                            USB_CDC_SET_LINE_CODING, 1 , deviceInfo[i].commInterface.interfaceNum , USB_CDC_LINE_CODING_LENGTH, (BYTE*)PTR_HOST_LINE_CODING_BUFFER,
                    ...
                    case SUBSTATE_WAIT_FOR_SET_LINE_CODING:   
                        if (USBHostTransferIsComplete( deviceInfo[i].deviceAddress, 0, &errorCode, &byteCount ))
                        {
                            ...
                            {    // check again if line coding is correctly set
                                 deviceInfo[i].state = STATE_INITIALIZE_DEVICE | SUBSTATE_GET_LINE_CODING;
                            }
                        }
                        break;
                   ...
                   case SUBSTATE_SET_CONTROL_LINE_STATE:
                       ...
                       if(!USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                            USB_CDC_SET_CONTROL_LINE_STATE , 0 , deviceInfo[i].commInterface.interfaceNum , USB_CDC_CONTROL_LINE_LENGTH, (BYTE*)PTR_HOST_CONTROL_LINE_BUFFER,
                                            USB_DEVICE_REQUEST_SET ))
                       {           
                            _USBHostCDC_SetNextSubState();
                       }
            ...
            case STATE_RUNNING:
                switch (deviceInfo[i].state & SUBSTATE_MASK)
                {
                    ...
                    case SUBSTATE_SEND_READ_REQ:   
                            ...
                            if(deviceInfo[i].endpointDATA == 0x00)
                            {
                                errorCode   = USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                                                    deviceInfo[i].commRequest, 0, deviceInfo[i].interface,deviceInfo[i].reportSize, deviceInfo[i].userData,
                                                                    USB_DEVICE_REQUEST_GET );
                            }
                            else
                            {
                    ...
                    case SUBSTATE_READ_REQ_WAIT:   
                              if (USBHostTransferIsComplete( deviceInfo[i].deviceAddress, deviceInfo[i].endpointDATA, &errorCode, &byteCount ))
                              {
                                if (errorCode)
                                {
                                    ...
                                    {
                                        USB_HOST_APP_EVENT_HANDLER(deviceInfo[i].deviceAddress,EVENT_CDC_NAK_TIMEOUT,NULL, 0);
                    ...
                    case SUBSTATE_SEND_WRITE_REQ:   
                            ...
                            // otherwise request is for data interface transfer
                              ...
                              {   // if transfer size is more than 64 Bytes, multiple transactions are used to transfer the data
                                  if(deviceInfo[i].remainingBytes > USB_CDC_MAX_PACKET_SIZE)
                                  {
                                    deviceInfo[i].bytesTransferred = USB_CDC_MAX_PACKET_SIZE; // max transaction size is 64 bytes
                                    deviceInfo[i].remainingBytes -= USB_CDC_MAX_PACKET_SIZE;
                                    errorCode = USBHostWrite( deviceInfo[i].deviceAddress, deviceInfo[i].endpointDATA,
                                                    deviceInfo[i].userData, deviceInfo[i].bytesTransferred );
                                    deviceInfo[i].userData = deviceInfo[i].userData + USB_CDC_MAX_PACKET_SIZE;
                                  }
                                  else
                                  {
                                    deviceInfo[i].bytesTransferred = deviceInfo[i].remainingBytes;
                                    deviceInfo[i].remainingBytes = 0;
                                    errorCode = USBHostWrite( deviceInfo[i].deviceAddress, deviceInfo[i].endpointDATA,
                                                    deviceInfo[i].userData, deviceInfo[i].bytesTransferred );
                                  }
                    ...
                    case SUBSTATE_WRITE_REQ_WAIT:   
                              if (USBHostTransferIsComplete( deviceInfo[i].deviceAddress, deviceInfo[i].endpointDATA, &errorCode, &byteCount ))
                              {
                                ...
                                else
                                {
                                    ...
                                    if(deviceInfo[i].remainingBytes != 0)
                                    {
            ...
            case STATE_CDC_RESET_RECOVERY:
                switch (deviceInfo[i].state & SUBSTATE_MASK)
                {
                    case SUBSTATE_SEND_RESET:   /* Not sure of rest request */
                            errorCode = USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE,
                                            USB_CDC_RESET, 0, deviceInfo[i].interface, 0, NULL, USB_DEVICE_REQUEST_SET );
...
BYTE USBHostCDCTransfer( BYTE deviceAddress,BYTE request , BYTE direction, BYTE interfaceNum, WORD size, BYTE *data , BYTE endpointDATA)
{
    ...
    deviceInfo[i].remainingBytes    = size;
    deviceInfo[i].interface         = interfaceNum;
    deviceInfo[i].endpointDATA      = endpointDATA;
    deviceInfo[i].commRequest       = request;       // invalid entry if DATA transfer is requested
    #ifdef DEBUG_MODE
        ...
        if(!direction)
            {
                if(deviceInfo[i].endpointDATA == 0x00)// if endpoint 0 then use control transfer for COMM interface request
                {
                    errorCode   = USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                                        request, 0, deviceInfo[i].interface,deviceInfo[i].reportSize, deviceInfo[i].userData,
                                                        USB_DEVICE_REQUEST_SET );
...
BOOL USBHostCDCTransferIsComplete( BYTE deviceAddress, BYTE *errorCode, BYTE *byteCount )
{
    ...
    // Find the correct device.
    for (i=0; (i<USB_MAX_CDC_DEVICES) && (deviceInfo[i].deviceAddress != deviceAddress); i++);
    if ((i == USB_MAX_CDC_DEVICES) || (deviceInfo[i].state == STATE_DETACHED))
    {
...
BOOL USBHostCDCEventHandler( BYTE address, USB_EVENT event, void *data, DWORD size )
{
    ...
    switch (event)
    {
        ...
        case EVENT_DETACH:           // USB cable has been detached (data: BYTE, address of device)
            ...
            for (i=0; (i<USB_MAX_CDC_DEVICES) && (deviceInfo[i].deviceAddress != address); i++);
            if (i < USB_MAX_CDC_DEVICES)
            {
                ...
                USB_HOST_APP_EVENT_HANDLER(deviceInfo[i].deviceAddress,EVENT_DETACH,NULL, 0);
           }
                                ...
                                else
                                {
                                    ...
                                    if(0 == memcmp(&CDC_DEV_LINE_CODING_Buffer, PTR_HOST_LINE_CODING_BUFFER, USB_CDC_LINE_CODING_LENGTH))
                                    {
                                       ...
                                       if(!USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                                            ...
                                                            USB_DEVICE_REQUEST_SET ))
                                       {           
                                            deviceInfo[i].state = STATE_WAIT_FOR_SET_CONTROL_LINE_STATE;
                                       }
                                       else
                                       {
                                    ...
                                    else
                                    {
                                       ...
                                       if(!USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                                            ...
                                                            USB_DEVICE_REQUEST_SET ))
                                       {           
                                            deviceInfo[i].state = STATE_WAIT_FOR_SET_LINE_CODING;
                                       }
                                       else
                                       {
                                            _USBHostCDC_TerminateTransfer(USB_CDC_COMMAND_FAILED);
                            ...
                            if (USBHostTransferIsComplete( deviceInfo[i].deviceAddress, 0, &errorCode, &byteCount ))
                            {
                                ...
                                {    // check again if line coding is correctly set
                                    ...
                                    {           
                                        ...
                                        deviceInfo[i].state     = STATE_WAIT_FOR_GET_LINE_CODING;
                                    }
                                    else
                                    {
                                        _USBHostCDC_TerminateTransfer(USB_CDC_COMMAND_FAILED);
                    ...
                    case STATE_READ_REQ_WAIT :
                              if (USBHostTransferIsComplete( deviceInfo[i].deviceAddress, deviceInfo[i].endpointDATA, &errorCode, &byteCount ))
                              {
                                ...
                                else
                                {
                                    ...
                                    if(deviceInfo[i].endpointDATA == 0x00)
                                    {
                                        USB_HOST_APP_EVENT_HANDLER(deviceInfo[i].deviceAddress,EVENT_CDC_COMM_READ_DONE,NULL, 0);
                                    }
                                    else
                                    {   // read request was for data interface
                                        USB_HOST_APP_EVENT_HANDLER(deviceInfo[i].deviceAddress,EVENT_CDC_DATA_READ_DONE,NULL, 0);
                                    }
                                }
                              }
#ifdef DEBUG_MODE
                    ...
                    case STATE_WRITE_REQ_WAIT :
                              if (USBHostTransferIsComplete( deviceInfo[i].deviceAddress, deviceInfo[i].endpointDATA, &errorCode, &byteCount ))
                              {
                                ...
                                else
                                {
                                    ...
                                    if(deviceInfo[i].endpointDATA == 0x00)
                                    {
                                        USB_HOST_APP_EVENT_HANDLER(deviceInfo[i].deviceAddress,EVENT_CDC_COMM_WRITE_DONE,NULL, 0);
                                    }
                                    else
                                    {   // read request was for data interface
                                        USB_HOST_APP_EVENT_HANDLER(deviceInfo[i].deviceAddress,EVENT_CDC_DATA_WRITE_DONE,NULL, 0);
                                    }
                                }
                              }
#ifdef DEBUG_MODE
...
BOOL USBHostCDCInitialize( BYTE address, DWORD flags )
{
    ...
    for (device = 0; (device < USB_MAX_CDC_DEVICES) && (deviceInfo[device].deviceAddress != 0); device++);
    ...
    descriptor = USBHostGetCurrentConfigurationDescriptor( address );
        ...
        while (i < ((USB_CONFIGURATION_DESCRIPTOR *)descriptor)->wTotalLength)
        {
            ...
            if (descriptor[i+1] == USB_DESCRIPTOR_INTERFACE)
            {
                ...
                {
                    deviceInfo[device].commInterface.interfaceNum  = descriptor[i+2];
                    deviceInfo[device].commInterface.noOfEndpoints = descriptor[i+4];
                     ...
                     while(USB_CDC_CS_INTERFACE == descriptor[i+1] )
                    {
                         switch(descriptor[i+2])
                        {
                             case   USB_CDC_DSC_FN_HEADER   :  
                                         deviceInfo[device].commInterface.Header_Fn_Dsc = 
                                                            (*((USB_CDC_HEADER_FN_DSC*)&descriptor[i]));
                                 break;

                             case   USB_CDC_DSC_FN_CALL_MGT : 
                                         deviceInfo[device].commInterface.Call_Mgt_Fn_Desc = 
                                                            (*((USB_CDC_CALL_MGT_FN_DSC*)&descriptor[i]));
                                 break;

                             case   USB_CDC_DSC_FN_ACM      :
                                         deviceInfo[device].commInterface.ACM_Fn_Desc = 
                                                            (*((USB_CDC_ACM_FN_DSC*)&descriptor[i]));
                                 break;

                             case   USB_CDC_DSC_FN_UNION    :
                                         deviceInfo[device].commInterface.Union_Fn_Desc = 
                    ...
                    if(descriptor[i+1] == USB_DESCRIPTOR_ENDPOINT)
                    {
                        ...
                        {
                            if (((descriptor[i+2] & 0x80) == 0x80) && (deviceInfo[device].commInterface.endpointIN == 0))
                            {
                                deviceInfo[device].commInterface.endpointIN           = descriptor[i+2];
                                deviceInfo[device].commInterface.endpointMaxDataSize  = ((descriptor[i+4]) |
                                                                                        (descriptor[i+5] << 8));
                                deviceInfo[device].commInterface.endpointPollInterval = descriptor[i+6];
                            }
                        }
                        i += descriptor[i];
                    }
                ...
                {
                    deviceInfo[device].dataInterface.interfaceNum  = descriptor[i+2];
                    deviceInfo[device].dataInterface.noOfEndpoints = descriptor[i+4];
                    ...
                    {   // Data Interface uses Bulk or Iscochronous endpoint
                        deviceInfo[device].dataInterface.endpointType = descriptor[i+3];
                    ...
                    {
                        deviceInfo[device].dataInterface.endpointIN           = endpointIN;
                        deviceInfo[device].dataInterface.endpointInDataSize   = endpointINsize;

                        deviceInfo[device].dataInterface.endpointOUT          = endpointOUT;
                        deviceInfo[device].dataInterface.endpointOutDataSize  = endpointOUTsize;
           ...
           USBHostCDCInitAddress( deviceInfo[device].deviceAddress,0 );
           // Send GET Line Coding request to the device 
           if(USBHostDeviceRequest( deviceInfo[device].deviceAddress, USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE, 
                                     USB_CDC_GET_LINE_CODING, 0 , deviceInfo[device].commInterface.interfaceNum , USB_CDC_LINE_CODING_LENGTH, (BYTE*)&CDC_DEV_LINE_CODING_Buffer,
                                     USB_DEVICE_REQUEST_GET ))
           {           
                return FALSE;
           }
           // wait for transfer event for and then decide to alter the settings or not
           deviceInfo[device].state                = STATE_WAIT_FOR_GET_LINE_CODING;
        #endif
       return TRUE;
}
...
void _USBHostCDC_ResetStateJump( BYTE i )
{
   ...
   if (deviceInfo[i].flags.bfReset)
    {
        #ifndef USB_ENABLE_TRANSFER_EVENT
            deviceInfo[i].state = STATE_CDC_RESET_RECOVERY;
        #else
             errorCode = USBHostDeviceRequest( deviceInfo[i].deviceAddress, USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_CLASS | USB_SETUP_RECIPIENT_INTERFACE,
                            USB_CDC_RESET, 0, deviceInfo[i].interface, 0, NULL, USB_DEVICE_REQUEST_SET );
    ...
    {
         USB_HOST_APP_EVENT_HANDLER(deviceInfo[i].deviceAddress,EVENT_CDC_RESET,NULL, 0);
