...
//******************************************************************************
//******************************************************************************
extern USB_CDC_DEVICE_INFO                  deviceInfo[USB_MAX_CDC_DEVICES] __attribute__ ((aligned));
...
BOOL USBHostCDC_Api_Get_IN_Data(BYTE no_of_bytes, BYTE* data)
{
    ...
    for (i=0; (i<USB_MAX_CDC_DEVICES) && (deviceInfo[i].deviceAddress != CDCdeviceAddress); i++);

   if(!USBHostCDCRead_DATA(CDCdeviceAddress, deviceInfo[i].dataInterface.interfaceNum,
                           no_of_bytes,data,deviceInfo[i].dataInterface.endpointIN))
    {
       return TRUE;
    }
    return FALSE;
...
BOOL USBHostCDC_Api_Send_OUT_Data(BYTE no_of_bytes, BYTE* data)
{
   ...
   if(!USBHostCDCSend_DATA(CDCdeviceAddress, deviceInfo[i].dataInterface.interfaceNum,
                           no_of_bytes,data,deviceInfo[i].dataInterface.endpointOUT))
    {
       return TRUE;
    }
    return FALSE;
    ...
    switch(requestType)
    {
        case USB_CDC_SEND_ENCAPSULATED_COMMAND : 
                               return_val = USBHostCDCTransfer(CDCdeviceAddress, USB_CDC_SEND_ENCAPSULATED_COMMAND,0,
                                                       deviceInfo[i].commInterface.interfaceNum,size,data,
                                                       0);
            break;
