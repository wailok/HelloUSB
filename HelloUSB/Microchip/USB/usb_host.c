...
BYTE USBHostSuspendDevice( BYTE deviceAddress )
{
    ...
    if (usbHostState != (STATE_RUNNING | SUBSTATE_NORMAL_RUN))
    {
        ...
        //Indicate To USB Module That It Is About To Be Suspended
        U1PWRCbits.USLPGRD = 1;
    #endif
        ...
        case STATE_DETACHED:
            switch (usbHostState & SUBSTATE_MASK)
            {
                case SUBSTATE_INITIALIZE:
                    ...
                    #else
                        U1OTGCON            = USB_DPLUS_PULLDOWN_ENABLE | USB_DMINUS_PULLDOWN_ENABLE | USB_VBUS_ON; // Pull down D+ and D-
                    #endif
...
#endif
{
    ...
    if (U1IEbits.DETACHIE && U1IRbits.DETACHIF)
    {
        ...
        if (U1IRbits.RESUMEIF && U1IEbits.RESUMEIE)
        {
            ...
            //Wait For Device To Finish Resume Signaling (TDRSMUPmax)
            USBOTGDelayMs(15);

            //Configure State Machine To Resume
            usbOverrideHostState = STATE_RUNNING | SUBSTATE_SUSPEND_AND_RESUME | SUBSUBSTATE_RESUME;
